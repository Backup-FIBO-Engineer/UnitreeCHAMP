...
steps are ramped at `gait.max_linear_acceleration` (B2 0.30 m/s²), the gait
starts with every foot on the ground, and a stop (or a reversal through zero)
waits for the swinging pair to land before the gait resets, so no foot is ever
commanded from swing height to the floor in one tick.
