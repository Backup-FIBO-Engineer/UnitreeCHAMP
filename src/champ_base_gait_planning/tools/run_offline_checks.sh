...
python3 tools/verify_mujoco_walk.py 0.35
...
g++ -std=c++17 -O2 -I include -o /tmp/verify_go2_lowcmd \
  tools/verify_go2_lowcmd.cpp src/motor_crc.cpp
/tmp/verify_go2_lowcmd
python3 - <<'PY'
from pathlib import Path
text = Path("config/go2_lowcmd.yaml").read_text()
ok = "motor_mode: 1" in text
print(f"[{'PASS' if ok else 'FAIL'}] go2_lowcmd motor_mode is 0x01 (sdk2 PMSM)")
