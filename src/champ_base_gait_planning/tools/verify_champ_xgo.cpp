// Offline verification of CHAMP IK/FK, gait, and odometry against XGO URDF
// translations. Compiles against champ headers only (no ROS).
//
// Build:
//   g++ -std=c++17 -O2
//     -I <repo>/cyclonedds_ws/src/motion_control/champ/include/champ
//     -o /tmp/verify_champ_xgo verify_champ_xgo.cpp
//
// Translations below are parent_to_joint_origin xyz from urdf/xgo_rviz.xacro
...
// Exact xyz from xgo_rviz.xacro joint origins (rpy all zero).
const LegXyz kLf = {
  {0.09332f, 0.022407f, 0.01454f},
  {-0.018325f, 0.049313f, 0.0f},
  {0.0f, -0.00072f, -0.059896f},
  {-0.028f, 0.0f, -0.0815f},
};
const LegXyz kRf = {
  {0.09332f, -0.022517f, 0.01454f},
  {-0.018325f, -0.049313f, 0.0f},
  {0.0f, 0.0f, -0.059896f},
  {-0.028f, 0.0f, -0.0815f},
};
const LegXyz kLh = {
  {-0.05668f, 0.022407f, 0.01454f},
  {-0.018325f, 0.0493130541805868f, 0.0f},
  {0.0f, -0.00072f, -0.059896f},
  {-0.028f, 0.0f, -0.0815f},
};
const LegXyz kRh = {
  {-0.05668f, -0.022517f, 0.01454f},
  {-0.018325f, -0.049313f, 0.0f},
  {0.0f, 0.0f, -0.059896f},
  {-0.028f, 0.0f, -0.0815f},
};
...
{
  // Matches config/xgo_rviz_gait.yaml. knee_orientation must outlive setGaitConfig.
  static const char kKnee[] = ">>";
  champ::GaitConfig gait;
  gait.knee_orientation = kKnee;
  gait.odom_scaler = 0.9f;
  gait.max_linear_velocity_x = 0.25f;
  gait.max_linear_velocity_y = 0.12f;
  gait.max_angular_velocity_z = 0.8f;
  gait.com_x_translation = 0.0f;
  gait.swing_height = 0.02f;
  gait.stance_depth = 0.0f;
  gait.stance_duration = 0.25f;
  gait.nominal_height = 0.10f;
  return gait;
}

void load_xgo(champ::QuadrupedBase & base)
{
...
int main()
{
  ...
  std::snprintf(buf, sizeof(buf), "l1+l2=%.4f m (yaml claims 0.146)", lf_reach);
  check(std::fabs(lf_reach - 0.146f) < 0.001f, "LF planar reach matches yaml", buf);

  const float l0 =
    base.lf.upper_leg.y() + base.lf.lower_leg.y() + base.lf.foot.y();
  std::snprintf(buf, sizeof(buf), "l0=%.5f (hip Y offset of planar chain)", l0);
  check(l0 > 0.04f && l0 < 0.06f, "LF l0 is outward hip offset", buf);
  ...
  std::snprintf(
    buf, sizeof(buf),
    "req_tz=%.4f (nominal 0.10 sits %.1f mm above fully-stretched Z)",
  ...
  // poseCommand target is hip-frame; foot_from_base.Z should be ~ -0.10
  ...
  far_feet[0].Z() -= 0.5f;  // 50 cm further down — beyond 0.146 m reach
  float before[12];
  ...
  const Case cases[] = {
    {"vx=0.25", 0.25f, 0.0f, 0.0f},
    {"vy=0.12", 0.0f, 0.12f, 0.0f},
    {"wz=0.8", 0.0f, 0.0f, 0.8f},
    {"vx+wz", 0.20f, 0.0f, 0.5f},
    {"worst yaml", 0.25f, 0.12f, 0.8f},
  };
