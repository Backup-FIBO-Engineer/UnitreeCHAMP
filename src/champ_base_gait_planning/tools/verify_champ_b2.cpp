// Offline verification of CHAMP IK/FK, gait, and odometry against B2 URDF
// translations. Compiles against champ headers only (no ROS).
//
// Build:
//   g++ -std=c++17 -O2
//     -I <repo>/src/champ/include/champ
//     -o /tmp/verify_champ_b2 verify_champ_b2.cpp
//
// Translations below are parent_to_joint_origin xyz from urdf/b2.urdf
...
// Exact xyz from b2.urdf joint origins (rpy all zero). The calf joints carry the
// official +-8.7e-5 m y offset and RR_foot_joint y=-8.6984e-05; CHAMP sums them
// into l0, so they are kept rather than rounded away.
const LegXyz kLf = {
  {0.3285f, 0.072f, 0.0f},
  {0.0f, 0.11973f, 0.0f},
  {0.0f, -8.6984e-05f, -0.35f},
  {0.0f, 0.0f, -0.35f},
};
const LegXyz kRf = {
  {0.3285f, -0.072f, 0.0f},
  {0.0f, -0.11973f, 0.0f},
  {0.0f, 8.6986e-05f, -0.35f},
  {0.0f, 0.0f, -0.35f},
};
const LegXyz kLh = {
  {-0.3285f, 0.072f, 0.0f},
...
{
  // Matches config/b2_gait.yaml. knee_orientation must outlive setGaitConfig.
  static const char kKnee[] = ">>";
  champ::GaitConfig gait;
  gait.knee_orientation = kKnee;
  gait.odom_scaler = 0.9f;
  gait.max_linear_velocity_x = 0.60f;
  gait.max_linear_velocity_y = 0.20f;
  gait.max_angular_velocity_z = 0.6f;
  gait.com_x_translation = 0.0f;
  gait.swing_height = 0.10f;
  gait.stance_depth = 0.0f;
  gait.stance_duration = 0.30f;
  gait.nominal_height = 0.50f;
  return gait;
}

void load_b2(champ::QuadrupedBase & base)
{
...
int main()
{
  ...
  std::snprintf(buf, sizeof(buf), "l1+l2=%.4f m (yaml claims 0.70)", lf_reach);
  check(std::fabs(lf_reach - 0.70f) < 0.001f, "LF planar reach matches yaml", buf);

  const float l0 =
    base.lf.upper_leg.y() + base.lf.lower_leg.y() + base.lf.foot.y();
  std::snprintf(buf, sizeof(buf), "l0=%.5f (hip Y offset of planar chain)", l0);
  check(l0 > 0.11f && l0 < 0.13f, "LF l0 is outward hip offset", buf);
  ...
  std::snprintf(
    buf, sizeof(buf),
    "req_tz=%.4f (nominal 0.50 sits %.1f mm above fully-stretched Z)",
  ...
  // poseCommand target is hip-frame; foot_from_base.Z should be ~ -0.50
  ...
  far_feet[0].Z() -= 1.2f;  // beyond 0.70 m reach
  float before[12];
  ...
  const Case cases[] = {
    {"vx=0.60", 0.60f, 0.0f, 0.0f},
    {"vy=0.20", 0.0f, 0.20f, 0.0f},
    {"wz=0.6", 0.0f, 0.0f, 0.6f},
    {"vx+wz", 0.30f, 0.0f, 0.4f},
    {"worst yaml", 0.60f, 0.20f, 0.6f},
  };
