...
// Robot-specific values (gains, motor mode) have no
...
std::string missingParam(const std::string & name, const std::string & hint)
{
  return std::string(kNodeName) + " parameter '" + name + "' is missing. " + hint +
         " Load config/<robot>_lowcmd.yaml (see unitree_rl_sim2real.launch.py robot:=<robot>).";
}
...
public:
      ...
      .automatically_declare_parameters_from_overrides(true))
  {
    ...
    RCLCPP_INFO(
      get_logger(),
      "IMU/joint measurements publish only on a new %s (not every LowCmd timer tick)",
      lowstate_topic_.c_str());
  }

private:
  // Joint names come from joints_map yaml, joint limits from the URDF,
  ...
  // Policy ticks at control_rate (50 Hz) while motors are served at publish_rate
  // (500 Hz): repeating each target is a staircase that a stiff joint PD
  // (kp 1000 on B2) turns into a torque step. Interpolating linearly between
  // the last two targets costs one command period of latency.
  ...
  void onTimer()
  {
    ...
    // Do not stamp a cached LowState as new: the policy IMU timeout
    // would never fire if we republished at 500 Hz.
    if (publish_measurements) {
      publishMeasurements(measurement_stamp, measured, measured_dq, imu);
    }
