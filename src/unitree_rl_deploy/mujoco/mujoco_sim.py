#!/usr/bin/env python3
"""MuJoCo physics simulation of a Unitree quadruped for policy deploy.

Robot-agnostic: the model file comes from the `xml_path` parameter, the joint
names from the `joints_map` yaml, the base/IMU frames from `links_map`
...
different set of config files, see mujoco_rl.launch.py robot:=<robot>.
...
class MujocoSim(Node):
    def __init__(self) -> None:
        # Same parameter style as the other nodes: everything from the yaml files
        ...
        self.joint_names = self._mapped_joint_names()
        ...
        if not configured_xml:
            raise ValueError(
                "parameter 'xml_path' is required (mujoco_rl.launch.py sets it to "
                'mujoco/<robot>.xml)'
            )
        ...
        # Optional slope through the origin (rad) for IMU / projected-gravity tests.
    ...
    def _require_string(self, name: str) -> str:
        ...
        if not isinstance(value, str) or not value:
            raise ValueError(f"parameter '{name}' is required (links/joints yaml)")
        return value
    ...
    def _require_string_list(self, name: str, minimum: int) -> List[str]:
        ...
        if value is None or len(value) < minimum:
            raise ValueError(
                f"parameter '{name}' must list at least {minimum} names (robot yaml)"
            )
        return [str(v) for v in value]

    def _mapped_joint_names(self) -> List[str]:
        """Actuated joints: LF, FR, RL, RR x (hip, thigh, calf) from joints_map."""
        names: List[str] = []
        for leg in LEGS:
    ...
    def command_callback(self, msg: JointState) -> None:
        ...
        with self.lock:
            if not self.started:
                # Put the model into the first commanded pose before enabling gravity.
    ...
    def _update_control_for_step(self, dt: float, now_monotonic: float) -> None:
        ...
        if timeout > 0.0 and now_monotonic - self.last_command_monotonic > timeout:
            # Hold the current measured pose if the policy command timed out.
